...
class SohotVisualization:
    ...
    def __init__(self, sohot, schema):
        ...
        self.data_path = "data/images_sohot"  + "/gif2"
    ...
    def visualize_soft_hoeffding_tree(self, X=None, print_idx=0, save_img=False, revert_transformation=None,
        ...
        if save_img:
            ...
            # plt.savefig(f"{self.data_path}/figure-tree-at-{print_idx}.png")       # store as png for gif
            plt.clf()
        else:
            plt.show()
    ...
    def get_tree_text(self, X=None, revert_transformation=None):
        """
        Return the complete Soft Hoeffding Tree as a readable string.
        ...
        ----------
        X:
            Current input instance. Needed for feature importance.
            If X is None, feature importance is not available.

        revert_transformation:
            Optional function used to transform split values back
            to their original scale.
        """

        lines = []

        def add_leaf(leaf, indent):
            # Same class distribution calculation as in the visualization
            probabilities = self.softmax(
                self.sohot.weights[leaf.orientation_sequence]
            ).detach().cpu().numpy()
            ...
            lines.append(
                f"{indent}  Class distribution: {{{distribution}}}"
            )
            lines.append(
                f"{indent}  Predicted class: {predicted_class}"
            )
            lines.append(
                f"{indent}  P(x->leaf): {leaf.sample_to_node_prob:.3f}"
            )
        ...
        def add_node(node, indent=""):
            ...
            split_test = self.get_node_label(
                feature_idx=node.split_test.feature,
                revert_transformation=revert_transformation,
                split_at_value=node.split_test.split_at,
                impact=""
            )

            lines.append(
                f"{indent}Split test: {split_test}"
            )
            ...
            impact = self.get_fi_impact(node, X)

            lines.append(
                f"{indent}Feature importance:"
            )

            if impact.strip():
                for importance_line in impact.strip().splitlines():
                    lines.append(
                        f"{indent}  {importance_line}"
                    )
            else:
                lines.append(
                    f"{indent}  n/a"
